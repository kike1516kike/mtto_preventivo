...
@section('content')
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#confirmDeleteModal{{ $ubicacion->id_ubicacion }}">Eliminar</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
            ...
            <div class="col-md-12 d-flex justify-content-center">
                <nav aria-label="Page navigation example">
                    <ul class="pagination">
                        ...
                        @else
                            <li class="page-item">
                                <a class="page-link" href="{{ $ubicaciones->previousPageUrl() }}"
                                    tabindex="-1">Anterior</a>
                            </li>
                        @endif

                        @foreach ($ubicaciones->getUrlRange(max(1, $ubicaciones->currentPage() - 2), min($ubicaciones->lastPage(), $ubicaciones->currentPage() + 2)) as $page => $url)
    ...
    @foreach ($ubicaciones as $ubicacion)
        <div class="modal fade" id="confirmDeleteModal{{ $ubicacion->id_ubicacion }}" tabindex="-1"
            aria-labelledby="confirmDeleteModalLabel{{ $ubicacion->id_ubicacion }}" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title text-dark" id="confirmDeleteModalLabel{{ $ubicacion->id_ubicacion }}">
                            Eliminar Marca</h5>
                    ...
                    <div class="modal-footer">
                        ...
                        <form action="{{ route('ubicaciones.destroy', $ubicacion) }}" method="post"
                            id="deleteForm{{ $ubicacion->id_ubicacion }}" class="frmDelete">
                            @csrf
                            @method('DELETE')
