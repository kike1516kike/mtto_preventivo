...
@section('content')
                ...
                <tbody>
                    @forelse ($perfiles as $perfil)
                        <tr>
                            ...
                            <td class="">{{ $perfil->cargo_perfil }}</td>
                            <td class="text-center">
                                ...
                                <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#confirmDeleteModal{{ $perfil->id_perfil }}">Eliminar</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
            ...
            <div class="col-md-12 d-flex justify-content-center">
                <nav arial-label="Page navegation example">
                    <ul class="pagination">
                        ...
                        @foreach ($perfiles->getUrlRange(max(1, $perfiles->currentPage() - 2), min($perfiles->lastPage(), $perfiles->currentPage() + 2)) as $page => $url)
    ...
    @foreach ($perfiles as $perfil)
        <div class="modal fade" id="confirmDeleteModal{{ $perfil->id_perfil }}" tabindex="-1"
            aria-labelledby="confirmDeleteModalLabel{{ $perfil->id_perfil }}" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title text-dark" id="confirmDeleteModalLabel{{ $perfil->id_perfil }}">Eliminar
                            Perfil</h5>
                    ...
                    <div class="modal-footer">
                        ...
                        <form action="{{ route('perfiles.destroy', $perfil) }}" method="post"
                            id="deleteForm{{ $perfil->id_perfile }}" class="frmDelete">
                            @csrf
                            @method('DELETE')
